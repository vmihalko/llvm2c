    ...
    if (callInst->getCalledFunction()) {
        ...
        */
        if (!funcName.substr(0,9).compare("llvm.smax") || !funcName.substr(0,9).compare("llvm.smin") ||
            ...
            func->createExpr(&ins, std::make_unique<SelectExpr>(cmprsn_ptr, a, b));
            block->addExpr(func->getExpr(&ins));
            return;
        }
...
static Expr* parseCastInstruction(const llvm::Instruction& ins, Program& program) {
    ...
    if (CI->getOperand(0)->getType()->isIntegerTy(1) &&
        ...
        // create zero
        auto zero = std::make_unique<Value>("0", program.getType(ins.getOperand(2)->getType()));
        // create -1
        auto minusOne = std::make_unique<Value>("-1", program.getType(ins.getOperand(2)->getType()));
        // create comparison select ? -1 : 0
        Expr* cond = program.getExpr(ins.getOperand(1));
